...
/*============================================================
submatrix returns a matrix structure object of input matrix m 
starting at index (start_i,start_j) and of size height x width
============================================================*/
matrix* submatrix(matrix* m,int start_i,int start_j, int width, int height){
    matrix* out = makeMatrix(width, height);
    start_i--;start_j--;
    double* ptrM = m->data + (start_i*m->width)+start_j;
    double* ptrOut = out->data;
    float buff;
    for (size_t i = 0; i < height; i++) {
        for (size_t j = 0; j < width; j++){
            *(ptrOut++) = *(ptrM+(i*m->width)+j);
        }
    }
    return out;
}